...
static class BuildCommand
{
    ...
    private const string IOS_BUILD_NUMBER = "VERSION_BUILD_VAR";
    ...
    static bool TryConvertToEnum<TEnum>(this string strEnumValue,
        out TEnum value)
    {
    ...
    public static bool TryGetEnv(string key,
        out string value)
    {
    ...
    private static void HandleIOSBundleVersionCode()
    {
        if (TryGetEnv(IOS_BUILD_NUMBER, out string value))
        {
            if (int.TryParse(value, out int version))
            {
                PlayerSettings.iOS.buildNumber = $"{version}";
                Console.WriteLine($":: {IOS_BUILD_NUMBER} env var detected, set the build number to {value}.");
            }
            else
                Console.WriteLine(
                    $":: {IOS_BUILD_NUMBER} env var detected but the version value \"{value}\" is not an integer.");
        }
    }
