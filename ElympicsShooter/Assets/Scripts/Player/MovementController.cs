...
public class MovementController : ElympicsMonoBehaviour
{
    ...
    private bool IsGrounded() => Physics.CheckSphere(CapsuleBottomSphereCenter + IsGroundedCheckOffset, checkSphereRadius, groundLayer);
    ...
    private void Awake()
    {
        ...
        checkSphereVerticalOffset = (physicsCollider.center.y - physicsCollider.height / 2.0f + physicsCollider.radius) * Vector3.up;
        checkSphereRadius = transform.localScale.y * physicsCollider.radius * GroundCheckSphereScale;
    }

    private void ResetVelocityIfMatchEnded(int _, int newGameState)
    {
    ...
    private void ApplyMovement(Vector3 movementDirection, bool isGrounded)
    {
        Vector3 targetVelocity = movementDirection * movementSpeed;
        Vector3 effectiveVelocity = Vector3.MoveTowards(rigidbody.velocity, targetVelocity, Elympics.TickDuration * acceleration);
    ...
    private Vector3 GetSlopeAdjustedVelocity(Vector3 currentVelocity)
    {
        ...
        Vector3 groundForward = Vector3.Cross(Vector3.Cross(groundContact.Norm, Vector3.up), groundContact.Norm).normalized;
        ...
        else
        {
            ...
            return ZeroY(currentVelocity + -velocityForwardComponent + velocityForwardComponent / Mathf.Pow(slopeAngleCos, 2));
        }
    }

    private GroundContact GetGroundContactProperties()
    {
        Vector3 currentCapsuleBottomSphereCenterPosition = CapsuleBottomSphereCenter;   // catched for minor optimaliztaion
        float minAngle = MaxClimbAngle;
        Vector3 chosenNorm = Vector3.zero;

        // get possible ground colliders
        var touchingColliders = Physics.OverlapSphere(currentCapsuleBottomSphereCenterPosition + IsGroundedCheckOffset, checkSphereRadius, groundLayer);
        ...
        {
            Vector3 norm = currentCapsuleBottomSphereCenterPosition - collider.ClosestPoint(currentCapsuleBottomSphereCenterPosition);
    ...
    private void ProcessJumping(bool jump, bool isGrounded)
    {
        if (jump)
        {
